    ...
    TS: TimeSource,
{
    pub fn new(sdcard: SdCard<SPI, DELAY>, time_source: TS) -> Self {
        println!("{}", "New SDCard Manager".green());
...
pub struct SDIoManager<SPI, DELAY, TS>
where
    SPI: SpiDevice,
    DELAY: DelayNs,
    TS: TimeSource,
{
    sd_manager: Rc<RefCell<SdCardManager<SPI, DELAY, TS>>>,
    pub graph_id: GraphId,
}

impl<SPI, DELAY, TS> SDIoManager<SPI, DELAY, TS>
where
    SPI: SpiDevice,
    DELAY: DelayNs,
    TS: TimeSource,
{
    pub fn new(sd_manager: SdCardManager<SPI, DELAY, TS>, graph_id: GraphId) -> Self {
        println!("{}", format!("New GraphId: {:?}", graph_id).green());

        let sd_manager = Rc::new(RefCell::new(sd_manager));
        SDIoManager {
            sd_manager,
            graph_id,
        }
    ...
    pub fn default(sd_manager: SdCardManager<SPI, DELAY, TS>) -> Self {
        ...
        ) -> Result<R, StorageError>,
    {
        println!(
            "{}",
            format!("Attempting to Access File: {}", file_name).green()
        );
        let volume_manager = &mut self.sd_manager.as_ref().borrow_mut().volume_mgr;
        let raw_volume = volume_manager.open_raw_volume(VolumeIdx(0)).map_err(|e| {
            println!("Error Opening Raw Volume: {:?}", format!("{:?}", e).red());
            StorageError::IoError
        })?;

        let mut root_directory = volume_manager
            .open_root_dir(raw_volume)
            .map(|root| root.to_directory(volume_manager))
            .map_err(|e| {
        ...
        volume_manager.close_volume(raw_volume).map_err(|e| {
            println!(
...
impl<SPI, DELAY, TS> linear::io::IoManager for SDIoManager<SPI, DELAY, TS>
where
    SPI: SpiDevice,
    DELAY: DelayNs,
    TS: TimeSource,
{
    ...
    fn create(&mut self, id: GraphId) -> Result<Self, StorageError> {
        ...
        self.with_file(&file_name, Mode::ReadWriteCreate, 0, |_| Ok(()))?;
        ...
        self.graph_id = id; // Set the graph_id
        Ok(self.clone())
    }
    ...
    fn open(&mut self, id: GraphId) -> Result<Option<Self>, aranya_runtime::StorageError> {
        ...
        let file_name = truncate_filename(format!("d_{}.b", id), 8);
        self.with_file(&file_name, Mode::ReadOnly, 0, |_| Ok(()))?;
        let file_name = truncate_filename(format!("h_{}.b", id), 8);
        self.with_file(&file_name, Mode::ReadOnly, 0, |_| Ok(()))?;
        let file_name = truncate_filename(format!("l_{}.b", id), 8);
        self.with_file(&file_name, Mode::ReadOnly, 0, |_| Ok(()))?;
...
impl<SPI, DELAY, TS> linear::io::Write for SDIoManager<SPI, DELAY, TS>
where
    SPI: SpiDevice,
    DELAY: DelayNs,
    TS: TimeSource,
{
    ...
    fn head(&self) -> Result<Location, StorageError> {
        ...
        println!("{}", format!("Current Graph: {:?}", self.graph_id).green());

        let file_name = truncate_filename(format!("h_{}.b", self.graph_id), 8);
        let head_location: Location = self.with_file(&file_name, Mode::ReadOnly, 0, |file| {
            let mut serialize_vec: Vec<u8> = Vec::new();
            while !file.is_eof() {
                let mut buffer: [u8; 16] = [0; 16];
                let characters_parsed = file.read(&mut buffer).map_err(|e| {
                    println!(
    ...
    fn append<F, U>(&mut self, builder: F) -> Result<U, StorageError>
    where
        F: FnOnce(usize) -> U,
        U: Serialize,
    {
        println!("{}", "Append".green());
        println!("{}", format!("Current Graph: {:?}", self.graph_id).green());
        println!("{}", "Graph Data Management".green());
        let file_name = truncate_filename(format!("d_{}.b", self.graph_id), 8);
        let data_file_length =
            self.with_file(&file_name, Mode::ReadOnly, 0, |file| Ok(file.length()))?;
        println!(
            "{}",
        ...
        let serialized_len = self.with_file(
            &file_name,
            Mode::ReadWriteAppend,
            data_file_length,
            |file| {
                let mut serialized = Vec::new();
                into_writer(&data, &mut serialized).map_err(|e| {
                ...
                file.write(&serialized).map_err(|_| StorageError::IoError)?;
                println!(
                    "{}",
                    format!("Serialized Graph Data Written: {:?}", serialized).blue()
                );
                Ok(serialized.len())
            },
        )?;
        ...
        let file_name = truncate_filename(format!("l_{}.b", self.graph_id), 8);
        let locations_vec = self.with_file(&file_name, Mode::ReadWriteAppend, 0, |file| {
            let mut serialized = Vec::new();
            while !file.is_eof() {
                let mut buffer: [u8; 16] = [0; 16];
                let characters_parsed =
                    file.read(&mut buffer).map_err(|_| StorageError::IoError)?;
                serialized.extend_from_slice(&buffer[..characters_parsed]);
            }

            println!(
                "{}",
                format!("Reading Locations Binary: {:?}", serialized).blue()
            );

            let mut locations_vec: Vec<usize> = if serialized.is_empty() {
                Vec::new()
            } else {
            ...
            let append_bytes_location =
                locations_vec.last().map_or(0, |&last| last) + serialized_len;
            ...
            file.seek_from_start(0).map_err(|e| {
                println!(
                    "Failed to set Cursor to Offset {} Bytes From the Start of File: {:?}",
                    0,
                    format!("{:?}", e).red()
            ...
            file.write(&serialized).map_err(|e| {
                println!(
    ...
    fn commit(&mut self, head: Location) -> Result<(), StorageError> {
        ...
        let file_name = truncate_filename(format!("h_{}.b", self.graph_id), 8);
        self.with_file(&file_name, Mode::ReadWriteAppend, 0, |file| {
            let mut serialized = Vec::new();
            while !file.is_eof() {
                let mut buffer: [u8; 16] = [0; 16];
                let characters_parsed =
...
impl<SPI, DELAY, TS> linear::io::Read for SDIoManager<SPI, DELAY, TS>
where
    SPI: SpiDevice,
    DELAY: DelayNs,
    TS: TimeSource,
{
        ...
        T: DeserializeOwned, // ! + Debug,
    {
        ...
        let mut locations_vec = self.with_file(&file_name, Mode::ReadWriteAppend, 0, |file| {
            let mut serialize_vec: Vec<u8> = Vec::new();
            while !file.is_eof() {
                let mut buffer: [u8; 16] = [0; 16];
                let characters_parsed =
            ...
            let locations_vec: Vec<usize> = if serialize_vec.is_empty() {
                Vec::new()
            } else {
                from_bytes(&serialize_vec).map_err(|e| {
                    println!(
        ...
        let deserialized_u =
            self.with_file(&file_name, Mode::ReadWriteAppend, offset as u32, |file| {
                let mut serialize_vec: Vec<u8> = vec![0; offset_data_end - offset];

                let characters_parsed = file
                ...
                /*println!(
                    "{}",
                    format!("deserialized_u: {:?}", format!("{:?}", deserialized_u)).blue()
                );*/// !
                Ok(deserialized_u)
            })?;
...
impl<SPI, DELAY, TS> Clone for SDIoManager<SPI, DELAY, TS>
where
    SPI: SpiDevice,
    DELAY: DelayNs,
    TS: TimeSource,
{
    fn clone(&self) -> Self {
        ...
        println!("{}", format!("Current graph: {:?}", self.graph_id).green());
        SDIoManager {
            sd_manager: self.sd_manager.clone(),
